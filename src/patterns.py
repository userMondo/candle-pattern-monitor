...
def format_price_report(tickers: List[Dict], timestamp: str = "") -> str:
    """
    Format price ticker data into a human-readable hourly price report.

    Args:
        tickers: List of price ticker dicts from BinanceClient.get_price_ticker()
        timestamp: Optional formatted timestamp for UTC+7 display
    ...
    lines = [
        f"📈 *Hourly Price Report*",
        f"",
        f"*Time (UTC+7):* `{timestamp}`",
        f"",
        f"_{'Pair':<12} {'Price':>14} {'24h Δ':>10} {'Range':>28} {'Vol':>12}_",
    ]
    ...
    for t in tickers:
        ...
        lines.append(
            f"`{symbol:<12} {price_str:>14} {arrow}{sign}{change:+.2f}% "
            f"H:{high:>10.2f} L:{low:>10.2f} V:{vol:>10.2f}`"
        )

    lines.append("")
    lines.append(f"_Summary: {len(tickers)} pairs monitored_")
    lines.append(f"_Report generated by Candle Pattern Monitor_")
